...
async def assign_role_to_user(
    ...
    try:
        ...
        logger.info(
            f"Role assigned: {assignment.role_name} to user {assignment.user_id} in org {org_id}"
        )
        ...
        for resource in all_resources:
            ...
            if actions1 != actions2:
                differences[resource] = {
                    role1: list(actions1),
                    role2: list(actions2),
                    f"only_in_{role1}": list(actions1 - actions2),
                    f"only_in_{role2}": list(actions2 - actions1),
                }
